...
function edition()
	{
		...
		echo '<table border="0">';
		global $bdd,$id,$lg; // r�cup�ration variables de connexion
			...
			$recuplg = mysql_query('SELECT * FROM lg ORDER BY id');
				while ($donneeslg = mysql_fetch_assoc($recuplg))
					...
					echo '<tr>';
								...
								echo '<input type="hidden" value="'.$donnees['id'].'" name="id">';	// champ formulaire cach� de l'ID
					...
					<table border="0" cellpadding="10" cellspacing="2" width="850">
						<tr>
							...
							<td valign="top">
								...
								<br><br><br>
<!-- *******************************************************************************************************  -->
<!-- Routine affichage formulaire et champs des cat�gories (selon langues en cours)  -->
<!-- *******************************************************************************************************  -->
										<?php edition(); ?>
								</center>
							</td>
