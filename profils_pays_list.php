...
function affichage_profil()
	{
		...
		global $lg,$niveau,$profil,$action,$action2,$ordre,$pays,$pageencours ; // récupération variable langue

		$requete = 'SELECT * FROM pays WHERE code="'.$pays.'"';
...
$chkpays="";
				...
				if ($ordre=="alpha") { 	$requete = 'SELECT * FROM profil WHERE niveau='.$niveau.' AND pays="'.$pays.'" ORDER BY utilisateur'; }

				if ($ordre=="dernier") { $requete = 'SELECT * FROM profil WHERE niveau='.$niveau.' AND pays="'.$pays.'" ORDER BY id DESC'; }

				if ($ordre=="premier") { $requete = 'SELECT * FROM profil WHERE niveau='.$niveau.' AND pays="'.$pays.'" ORDER BY id'; }

				$recup = mysql_query($requete);
				while ($data = mysql_fetch_assoc($recup))
					{
					if ($datapays['code']<>$chkpays) {
						echo '<tr>';
							echo '<td>';
								echo '&nbsp;';
							echo '</td>';
