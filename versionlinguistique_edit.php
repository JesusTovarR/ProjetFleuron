...
function edition_versionlinguistique()
	{
		echo '<form name="FormName" action="versionlinguistique_edit_enre.php" method="post">';
		echo '<table border="0">';
		global $bdd,$id,$lg; // récupération variables de connexion

		$recup = mysql_query('SELECT * FROM versionlinguistique WHERE id='.$id);
		...
		while ($donnees = mysql_fetch_assoc($recup))
			...
			$recuplg = mysql_query('SELECT * FROM lg ORDER BY id');
				while ($donneeslg = mysql_fetch_assoc($recuplg))
					...
					echo '<tr>';
						echo '<td width="50">';
							if ($donneeslg['code']=="fr")	// affichage de l'ID du  mot uniquement sur la version française
								{
								echo $donnees['id'];
								}
...
<br><br><br>
	<table border="0" cellpadding="0" cellspacing="5" width="600">
				...
				<?php edition_versionlinguistique(); ?>
	</table>
</center>
