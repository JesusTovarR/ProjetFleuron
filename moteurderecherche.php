...
//************************************************************************
//		enregistrement mot recherch� (ou compteur) si $compteurcon>0
//************************************************************************
...
//			v�rification si le mot a d�j� �t� recherch�
		...
		if ($nbmot>0) {
			...
			$requete ='INSERT INTO `recherche` (`id`,`mot`,`visiteur`) VALUES (NULL,'; // d�but de la composition de la requete de mise � jour
...
//************************************************************************
//		compteur des ressources trouv�es
//************************************************************************
...
if ($lg=="") {
	...
	$ok=0;
				...
				$position = stripos(strtolower($zone),strtolower($motcle)); // position du mot cl�
				if ($position>-1) {
					$av = substr($zone,$position-1,1); // caract�re pr�c�dent le mot cl�
					$apr = substr($zone,$position+strlen($motcle),1); // caract�re suivant le mot cl�
//echo '-apr='.$apr.'->';
...
//************************************************************************
//		Affichage des ressources trouv�es
//************************************************************************
function affichage_ressourcelist($motcleorigine)
	...
	$ok=0;
				$zone = " ".$data['typemedia']." ".$data['nom_'.$lg]." ".$data['nom_fr']." ".$data['description_'.$lg]." ".$data['description_fr']." ";
				$position = stripos(strtolower($zone),strtolower($motcleorigine)); // position du mot cl�
				if ($position>-1) {
//echo $zone.'<br>';
					$av = substr($zone,$position-1,1); // caract�re pr�c�dent le mot cl�
					$apr = substr($zone,$position+strlen($motcleorigine),1); // caract�re suivant le mot cl�
//echo '-apr='.$apr.'->';
...
//echo '--totalOk='.$ok.'------';
					...
					if ($compteurressources==0) {
						echo'<br><br><br>';
						echo '<tr><td><span class="texte_defaultGras">Aucun r�sultat trouv�</span>';
					}

	}

//************************************************************************
//		Affichage des notes trouv�es
//************************************************************************
			...
			<form name="FormName" action="moteurderecherche_tmp.php" target="_top" method="post">
				<table border="0" cellpadding="5" cellspacing="2" bgcolor="<?php echo couleur(2); // couleur clair ?>" >
					<tr>
						...
						<td colspan="2" align="center">
							<input type="text" name="motcle" size="24" value="<?php echo $motcleorigine //Mot cl� ?>">
						</td>
		...
		<table border="1" class="table" width="880">
			<tr>
				<td align="center" class="grise">
					<span class="texte_defaultGras"><?php echo new_versionlinguistique("text118") //� gauche du mot ?></span>
				</td>
				<td align="center" width="50" class="grise">
					<span class="texte_defaultGras"><?php echo new_versionlinguistique("text119") //Mot recherch� ?></span>
				</td>
				<td align="center" class="grise">
					<span class="texte_defaultGras"><?php echo new_versionlinguistique("text120") //� droite du mot ?></span>
				</td>
			</tr>
				<?php 
					if ( strlen($motcle)>0) {
				...
				<?php 
					if ($compteur==0) {
						echo'<br><br><br>';
						echo '<span class="texte_defaultGras">Aucun r�sultat trouv�</span>';
					}
				?>
		</center><br>
