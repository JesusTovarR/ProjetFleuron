...
function affichage_favoris()
	{
		...
		$nb=0;
			...
			$requete = 'SELECT * FROM favoris WHERE profil='.$idprofil.' ORDER BY jour DESC';
			$recup = mysql_query($requete);
				while ($favoris = mysql_fetch_assoc($recup))
					{
						affichage_ressource($favoris['ressource']);
					}
			echo '</table>';
	}
...
<!-- Partie centrale -->
							<td valign="top">
								...
								<br>
								<p>
									<?php affichage_favoris() // affichage ressources favoris?>
